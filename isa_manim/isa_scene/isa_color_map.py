...
class IsaColorMap:
    """
    This class is used to map colors for ISA objects by allocating colors in the color
    scheme.

    Attributes:
        colormap_default_color: Default color, used by registers, function units and
            memory units. Default is WHITE.
        ...
        _colormap_hash_dict: Dictionary of objects and their color. Key of the
            dictionary is the hash value and the value is assigned color.
    """
    ...
    def __init__(self, default_color: Color = WHITE, color_scheme: List[Color] = None):
        ...
        self._colormap_color_list: List[Color] = (
            color_scheme
            if color_scheme else [RED, BLUE, GREEN, YELLOW, TEAL, PURPLE, MAROON])
    ...
    def colormap_get_color(
        ...
        -   If the hash value exists in the color map, return the assigned color from
            dictionary.
        ...
        Returns:
            If `num` is 1, return a single color. If `num` is larger than 1, return a
                list of colors.
        """
        # If the hash value exists in the color map, return the assigned color from
        # dictionary.
            ...
            if num == 1:
                ...
                if isinstance(assign_color, list):
                    if len(assign_color) == num:
                        ...
                        return [
                            assign_color[i % len(assign_color)] for i in range(0, num)
                        ]
                else:
        ...
        # Otherwise, return the next color(s) in the color scheme.
            ...
            for _ in range(0, num):
                self._colormap_color_index = (self._colormap_color_index + 1) % len(
                    self._colormap_color_list)
                assign_color.append(
                    self._colormap_color_list[self._colormap_color_index])
            # Reshape the return color(s)
            if num == 1:
