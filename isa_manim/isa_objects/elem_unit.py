...
class ElemUnit(VGroup):
    ...
    def __init__(
        ...
        # If some MSB bits are specified to be zero, two rectangle is used to describe
        # one element.
        # One covers the LSB bits, while the other covers the zero MSB bits.
        if high_bits > 0 and high_zero:
            # Register rectangle
            self.elem_rect: Rectangle = Rectangle(
                color=color, height=1.0, width=width * get_scene_ratio())
            fill_elem_rect = Rectangle(
                color=color, height=1.0, width=(width - high_bits) * get_scene_ratio(),
                fill_opacity=fill_opacity)
            fill_elem_rect.move_to(
                self.elem_rect.get_right() + LEFT *
                ((width - high_bits) * 0.5) * get_scene_ratio())

            # Value text
            self.value_text: Text = Text(
                self._value2str(), color=color, font_size=font_size)
            self.value_text.move_to(fill_elem_rect.get_center())

            fill_value_text = Text(text="0", color=color, font_size=font_size)
            fill_value_text.move_to(
                self.elem_rect.get_left() + RIGHT * high_bits * 0.5 * get_scene_ratio())
            # Scale text
    ...
    def _value2str(self) -> str:
        ...
        if self.elem_value is None:
            return ""
        elif isinstance(self.elem_value, bool):
            return "T" if self.elem_value else "F"
    ...
    # Get locations.
    def get_elem_pos(self, offset: int, elem_width: int) -> np.ndarray:
        """
        Return the center position of another element that is right-aligned with current
        element.
        ...
        return (
            self.elem_rect.get_right() + LEFT * offset * get_scene_ratio() +
