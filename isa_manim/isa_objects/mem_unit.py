...
from manim import (
    VGroup, Text, Rectangle, DashedVMobject, RoundedRectangle, Triangle, CubicBezier,
...
class MemoryUnit(VGroup):
    ...
    Attributes:
        ...
        mem_map_text: List of text of memory map ranges. Each item in the list is a pair
            of loweset and highest address.
    ...
    def __init__(
        ...
        Args:
            ...
            mem_range: List of memory range, each item is a pair of lowest address and
                highest address.
            font_size: Font size of memory name.
            value_format: Format string for result values. Inherented by element units.
            para_enable: Whether the animation related to this memory unit can perform
                parallel or not.
            mem_map_width: Hint for width of memory map width.
            dual_port: Memory unit has two data port.
        """
        self.require_serialization = not para_enable and not dual_port
        ...
        self.para_enable: bool = para_enable
        self.dual_port: bool = dual_port
        ...
        if self.dual_port:
            ...
            if (data_width * 2 + 1) < addr_width:
                self.mem_map_width: int = max(
                    (addr_width + addr_width) * get_scene_ratio() + 6, mem_map_width)
            else:
                self.mem_map_width: int = max(
                    (addr_width + 2 * data_width) * get_scene_ratio() + 1 + 6,
                    mem_map_width)
        else:
            # 6 is width of mem_rect (4) + width of splitter (1 * 2).
            if data_width < addr_width:
                ...
                self.mem_map_width: int = max(
                    (addr_width + data_width) * get_scene_ratio() + 6, mem_map_width)

        # memory rectangle
        self.mem_rect: RoundedRectangle = RoundedRectangle(
            color=color, height=3.0, width=4)
        ...
        if status_width > 0:
            self.status_rect: Rectangle = DashedVMobject(
                Rectangle(
                    color=color, height=1.0, width=status_width * get_scene_ratio()))
        ...
        self.addr_label_text = Text(
            text="Addr", color=color,
            font_size=font_size).move_to(self.addr_rect.get_center() + UP)

        # Data rectangle
        self.data_rect: Rectangle = DashedVMobject(
            Rectangle(color=color, height=1.0, width=data_width * get_scene_ratio()))
        if self.dual_port:
            self.data_rect2: Rectangle = DashedVMobject(
                Rectangle(
                    color=color, height=1.0, width=data_width * get_scene_ratio()))

        if self.dual_port:
            self.data_rect.shift(
                RIGHT * (
                    1 + self.mem_rect.width / 2 + self.data_rect.width / 2 +
                    self.data_rect.width + 1))
            self.data_label_text = Text(
                text="Data1", color=color,
                font_size=font_size).move_to(self.data_rect.get_center() + UP)

            self.data_rect2.shift(
                RIGHT * (1 + self.mem_rect.width / 2 + self.data_rect.width / 2))
            self.data_label_text2 = Text(
                text="Data2", color=color,
                font_size=font_size).move_to(self.data_rect2.get_center() + UP)
        else:
            self.data_rect.shift(
                RIGHT * (1 + self.mem_rect.width / 2 + self.data_rect.width / 2))
            self.data_label_text = Text(
        ...
        super().__init__()
        self.add(
            self.mem_rect, self.name_text, self.addr_rect, self.addr_label_text,
            self.data_rect, self.data_label_text, *self.mem_map_list,
            *self.mem_map_text, self.mem_map_left_brace, self.mem_map_right_brace)
        if self.dual_port:
            self.add(self.data_rect2, self.data_label_text2)
        if self.status_rect:
    ...
    def get_data2_pos(self, width: int) -> np.ndarray:
        """
        Return center position of data port.
        ...
        Args:
            width: Bit width of data.
        """
        return self.data_rect2.get_right() + LEFT * width * get_scene_ratio() / 2
    ...
    def _index_of_mem_range(self, addr: int) -> int:
        """
        Check whether given address is covered in memory map ranges, and return the
        index of matched memory map range.

        Args:
            addr: Address to check.

        Returns:
            If the specified address is covered in one memory map range, return the
            index of matched memory map range. Otherwise, return -1.
        """
        idx = 0
        for laddr, raddr in self.mem_range:
            if laddr <= addr < raddr:
    ...
    def get_addr_mark(self, addr: int, color: Color) -> Triangle:
        ...
        addr_pos = (
            mem_range_rect.get_right() - (addr_offset * scale_factor) * RIGHT +
            UP * 0.6)

        # Create address mark and move to position.
        addr_mark = (
            Triangle(color=color).scale(0.2).rotate(60 * DEGREES).move_to(addr_pos))
    ...
    def get_rd_mem_mark(self, laddr: int, raddr: int, color: Color) -> Rectangle:
        ...
        The height of the memory mark is 0.34 (1/3 of the height of memory map
        rectangle).
    ...
    def get_wt_mem_mark(self, laddr: int, raddr: int, color: Color) -> Rectangle:
        ...
        The height of the memory mark is 0.66 (2/3 of the height of memory map
        rectangle).
        ...
        # Create memory mark and move to position.
        mem_mark = Rectangle(
            color=color, height=0.66, width=addr_range * scale_factor,
