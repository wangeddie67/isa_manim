    ...
    def construct_isa_flow(self):
        ...
        for i in range(0, 4):
            ...
            for j in range(0, 2):
                ...
                prod = self.function_call(
                    func="*(a, b)", args=[opa, opb], res_size=esize, color_hash="prod" + str(j),
                    isa_hash="prod" + str(j), args_value=["a", "b"])
                sum__ = self.function_call(
                    func="+(a, b)", args=[prod, sum_], res_size=esize, color_hash="add" + str(j),
                    isa_hash="add" + str(j), args_value=["a", "b"])
                sum_ = sum__
        ...
        for i in range(0, 4):
            ...
            for j in range(0, 4):
                ...
                prod = self.function_call(
                    func="*(a, b)", args=[opa, opb], res_size=esize, color_hash="prod",
                    isa_hash="prod" + str(j), args_value=["a", "b"])
                sum__ = self.function_call(
                    func="+(a, b)", args=[prod, sum_], res_size=esize, color_hash="add",
                    isa_hash="add" + str(j), args_value=["a", "b"])
                sum_ = sum__
