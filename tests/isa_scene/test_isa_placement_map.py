...
from isa_manim import (
    MovingCameraScene, config, get_config, DEFAULT_FONT_SIZE, WHITE, RIGHT, DOWN,
    RegUnit, FunctionUnit, MemoryUnit, Dot, NumberPlane, IsaPlacementMap)
...
class TestIsaPlacementMap(MovingCameraScene):
    ...
    def construct(self):
        ...
        zn = RegUnit(
            ["Zn"], WHITE, 128, 1, 1, None, DEFAULT_FONT_SIZE,
            get_config("elem_value_format"))
        zm = RegUnit(
            ["Zm"], WHITE, 128, 1, 1, None, DEFAULT_FONT_SIZE,
            get_config("elem_value_format"))
        za = RegUnit(
            ["Za"], WHITE, 256, 8, 2, None, DEFAULT_FONT_SIZE,
            get_config("elem_value_format"))

        mul_unit = FunctionUnit(
            "*(a,b)", WHITE, [16, 16], [32], ["a", "b"], [""], DEFAULT_FONT_SIZE,
            get_config("elem_value_format"), None)
        add_unit = FunctionUnit(
            "+(a,b)", WHITE, [32, 32], [32], ["a", "b"], [""], DEFAULT_FONT_SIZE,
            get_config("elem_value_format"), None)
        ...
        dots = [
            ...
            Dot(
                animation_map.get_placement_width() * RIGHT +
                animation_map.get_placement_height() * DOWN)
        ]

        grids = NumberPlane(
            x_range=(0, animation_map.get_placement_width(), 1),
            y_range=(-animation_map.get_placement_height(), 0,
                     1)).move_to(animation_map.get_placement_origin())
