...
from isa_manim import SingleIsaScene, set_config
...
class TestSingleIsaSceneMem(SingleIsaScene):
    ...
    def construct_isa_flow(self):
        ...
        # Memory unit.
        addr_val = [
            rn_reg.reg_value + rm_reg.reg_value * mbytes + e * mbytes
        ...
        self.decl_memory(
            addrlen, memsize, [mem_range], status_width=esize // 8, para_enable=True)
        ...
        for e in range(0, elements):
            data, status = self.read_memory(
                base, memsize, offset=mbytes * e, has_status_output=True)
