...
class TestMemoryUnitStatus(Scene):
    ...
    def construct(self):
        mem_unit = MemoryUnit(WHITE, 64, 32, 64, [[0, 0x1000], [0x4000, 0x5000]],
                              DEFAULT_FONT_SIZE, get_config("elem_value_format"), False, 4, 0)
        ...
        status_rect_label = Text("status_rect", color=YELLOW) \
            .move_to(mem_unit.mem_rect.get_left() + RIGHT * 4 + UP * 4.5)
        status_rect_arrow = Arrow(
            status_rect_label.get_bottom(), mem_unit.status_rect.get_top(), color=YELLOW)
